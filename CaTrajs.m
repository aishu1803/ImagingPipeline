function [res,perf_go,perf_nogo] = CaTrajs(trials,npoff)
...
for i = 1:75
perf_go(rn,i) = length(find(class_go(i,:)-zeros(1,size(class_go,2))))*100/size(class_go,2);
end
for i = 1:75
perf_nogo(rn,i) = length(find(class_nogo(i,:)-ones(1,size(class_nogo,2))))*100/size(class_nogo,2);
end
subplot(2,4,5)

function [go_dat,nogo_dat] = CaTrajs(trials,npoff,ww)

count = 1;
for i = 1:length(trials)
    if trials(i).reward
    dat(count).trialId = trials(i).nogo;
    dat(count).y = trials(i).C_raw(setdiff(1:size(trials(1).C_raw,1),npoff),trials(i).nosepokeframe-29:trials(i).nosepokeframe+30);
...
% res = neuralTraj('rat3_withoutte',dat);

% [estParams, seqTrain,seqTest] = postprocess(res);
% for i = 1:length(seqTrain)
%    x = smooth(seqTrain(i).xorth(2,:));
%     y= smooth(seqTrain(i).xorth(2,:));
% z= smooth(seqTrain(i).xorth(3,:));
% X = [x y z];
% [L,R,K] = curvature(X);
% for j = 1:size(K,1)
%     le_vec_length(j) = sqrt(K(j,1)^2 + K(j,2)^2+K(j,3)^2');
% end
% [b,ind] = sort(le_vec_length,'descend');
% ee = diff(ind(3:end));
% ll = find(abs(ee)>5)+3;
% ind_interesting(i,:) = ind([3 ll(1) ll(2)]);
% h = plot3(X(:,1),X(:,2),X(:,3));
% grid on;
% set(gca,'Box','on');
% hold on
% plot3(X(ind_interesting(i,:),1),X(ind_interesting(i,:),2),X(ind_interesting(i,:),3),'LineStyle','none','Marker','.','Color','red','MarkerSize',24);
% end

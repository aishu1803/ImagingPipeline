function [perf_traj,perf_raw,perf_overall_gng] = Decode(W,firingRates,trialNum,shuf,dims)
...
for i = 1:2
    for j = 1:2
        for k = 1:trialNum(1,i,j)
            top_3_proj(:,i,j,:,k) = W(:,dims)'*squeeze(firingRates(:,i,j,1:74,k));
            for l = 1:length(dims)
...
z_score_firingRatesAverage = nanmean(z_score_firingRates,1);

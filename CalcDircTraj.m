function [go_curves,nogo_curves,go_err_curves,nogo_err_curves,vel_nogo,vel_go,vel_nogo_err,vel_go_err] = CalcDircTraj(trajectories,trialNum,x)
nogo_curves = [];
go_curves = [];
go_err_curves=[];
nogo_err_curves=[];
for i = 1:size(trajectories,5)
    ...
    z_nogo = squeeze(trajectories(3,:,1,1,i));
...
for i = 1:size(trajectories,5)
    ...
    z_go = squeeze(trajectories(3,:,2,1,i));
    t(1) = nan;
...
for i = 1:length(nogo_curves)
    ...
    nogo_curves(i).chdirfinal2 = tmp_chdirfinal2 + 29;
...
for i = 1:length(go_curves)
    ...
    [~,tmp_chdirfinal2] = min(abs(tmp(30:55)));
    go_curves(i).chdirfinal2 = tmp_chdirfinal2 + 29;

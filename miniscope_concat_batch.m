...
function miniscope_concat_batch

clear all

% ask user how many folders to read to run sequentially
num_dir = input('Number of main directories you want to concatenate sequentially: ');

% ask use about framerate
fps = input('What is the framerate you recorded with (fps)?: ');

% ask user for filename
file_nm = input('What name addition do you want to add to .tif file besides animal name: ', 's');
vid_nm = [file_nm, '.tif'];
...
for d = 1:length(main_dir)
    ...
    fprintf('Folder that is being concatenated:\n %s\n', main_dir{d})
    
    clearvars -except d dialog_title file_nm frame_start main_dir num_dir vid_nm fps
    cd(main_dir{d})
    ...
    % find animal's name
    animal_nm_split = strsplit(main_dir{d},'\');       % add animal name to file
    animal_nm = cell2mat(animal_nm_split(length(animal_nm_split)-2));   % same
